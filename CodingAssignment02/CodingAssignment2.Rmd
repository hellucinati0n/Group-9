...
```{r q3}
# Define new client profile
new_client <- data.frame(
  Age = 35, 
  BMI = 26, 
  Children = 1, 
  Female = 1,  # 1 for female
  Smoker = 0,  # 0 for non-smoker
  WinterSprings = 0,  # City is Sanford, so all city dummies are 0
  WinterPark = 0,
  Oviedo = 0
)

# Predict charges for the new client with a 95% confidence interval
prediction <- predict(model, new_client, interval = "confidence", level = 0.95)
...
```
Fit (Predicted Value): This is the predicted medical expense for the new client.
Lwr (Lower Bound): The lower bound of the 95% confidence interval.
Upr (Upper Bound): The upper bound of the 95% confidence interval
