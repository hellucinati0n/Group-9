...
As shown in the graphs below, charges and children do not show a linear regression because the histograms are not normally distributed. Therefore, we need to train the logarithmic and quadratic models with a sample size of 10 on the variables charges and children before we can confirm that the regression fits a quadratic or logarithmic model. Since we can only plot quantitative variables in scatter-plot and histogram, we will exclude the categorical variables.
...
Data Transformation:
Transform Charges into the log function to get the points closer to each other. After transforming charges into the log function, it is shown on the histogram below that the log of charges has a normal distribution. Therefore, log of charges is a better representation of this variable.
...
```{r}
#Model 1
model_1 <- lm(lnCharges ~., data = train[,c(10,2:9)] ) #pulling specific columns
summary(model_1)
...
```{r}
#Model 2
model_2 <- lm(lnCharges ~., data = train[,c(10,3:9,15)] ) #pulling specific columns
summary(model_2)
...
```{r}
#Model 3
model_3 <- lm(lnCharges ~., data = train[,c(10,14,3:9)] ) #pulling specific columns
summary(model_3)
...
```{r}
#Model 4
model_4 <- lm(lnCharges ~., data = train[,c(10,2,4:9,17)] ) #pulling specific columns
summary(model_4)
...
```{r}
#Model 5
model_5 <- lm(lnCharges ~., data = train[,c(10,2,4:9,16)] ) #pulling specific columns
summary(model_5)
...
```{r}
#Model 6
model_6 <- lm(lnCharges ~., data = train[,c(10,2:4,6:9,12,5)] ) #pulling specific columns
summary(model_6)
...
#Model 7
model_7 <- lm(lnCharges ~., data = train[,c(10,2:4,6:9,18)] ) #pulling specific columns
summary(model_7)
