...
```{r q4}
par(mfrow=c(1,2))
hist(insurance$Children) #before
scatterplotMatrix(train[-c(4,6:9)])
summary(train)
```
Below, we will take both the log and quadratic forms of each variable to determine best fit relative to the linear form, which was found to be problematic.

Taking logs will bring outliers closer to other charges. After transforming charges into the log function, the histograms below show that the adapted version brings charges closer to a normal distribution. Therefore, log of charges creates a better model.
...
# raw charges vs quadratic charges
```
```{r}
...
```{r}
train$ChildrenSquared <- train$Children^2 # quadratic of children
...
train$AgeSquared <- train$Age^2 # quadratic of age
hist(train$AgeSquared) #after
summary(train$AgeSquared)
```
...
Transforming the "Children" variable does not give us a normal distribution and therefore yields bias and inefficiency for the aforementioned log and quadratic forms.



```{r}
#Model 1
model_1 <- lm(lnCharges ~., data = train[,c(10,2:9)] ) #pulling specific columns for log of charges
summary(model_1)
...
```{r}
#Model 2
model_2 <- lm(lnCharges ~., data = train[,c(10,3:9,15)] ) #pulling specific columns for lnCharges and lnAge
summary(model_2)
...
```{r}
#Model 3
model_3 <- lm(lnCharges ~., data = train[,c(10,14,3:9)] ) #pulling specific columns for lnCharges and quadratic Age
summary(model_3)
...
```{r}
#Model 4
model_4 <- lm(lnCharges ~., data = train[,c(10,2,4:9,17)] ) #pulling specific columns for lnCharges and lnBMI
summary(model_4)
...
```{r}
#Model 5
model_5 <- lm(lnCharges ~., data = train[,c(10,2,4:9,16)] ) #pulling specific columns for lnCharges and quadratic BMI
summary(model_5)
...
```{r}
#Model 6
model_6 <- lm(lnCharges ~., data = train[,c(10,2:4,6:9,12,5)] ) #pulling specific columns for lnCharges and quadratic Children
...
#Model 7
model_7 <- lm(lnCharges ~., data = train[,c(10,2:4,6:9,18)] ) #pulling specific columns for lnCharges, using children as a dummy variable
summary(model_7)
...
test$ChildrenSquared <- test$Children^2
...
test$model_6_pred <- predict(model_6,newdata = test) %>% exp()
...
test$error_6 <- test$model_6_pred - test$Charges
```
...
# Model 2
mape(test$error_6, test$saleprice)
```
